...
export function patch(oldNode,vnode){//既有初始化的功能，又有更新的逻辑
    ...
    if(isRealElement){
        ...
        return newElm
    }else{//oldVNode也是虚拟节点，比较两个虚拟节点，更新有变化的部分
...
function patchVnode(oldNode,vnode){
        ...
        if(!isSameVnode(oldNode,vnode)){//元素节点
...
function updateChild(el,oldChildren,newChildren){
    ...
    while(oldStartIndex<=oldEndIndex&&newStartIndex<=newEndIndex){
        if(!oldStartNode){//乱序时会置空移走的节点
            oldStartNode=oldChildren[++oldStartIndex]
        }else if(!oldEndNode){//乱序时会置空移走的节点
            ...
            oldStartNode=oldChildren[++oldStartIndex]
            newEndNode=newChildren[--newEndIndex]
        }else {//乱序，根据老列表做一个映射，用新的去找，找到则移动，找不到则添加，最后删除多余的
